...
public final class Death implements DeathHandler {
    ...
    private ResourceLocation getEntityBiomeId() {
        Biome biome = livingEntity.getCommandSenderWorld().getBiomeManager().getBiome(new BlockPos(livingEntity.position()));
    ...
    private void createEntity(EntityType<?> entityType) {
        ...
        newEntity.yHeadRot = newEntity.yRotO;
        newEntity.yBodyRot = newEntity.yRotO;
        ...
        newEntity.absMoveTo(livingEntity.position().x(), livingEntity.position().y(), livingEntity.position().z(), livingEntity.yRot, livingEntity.xRot);
