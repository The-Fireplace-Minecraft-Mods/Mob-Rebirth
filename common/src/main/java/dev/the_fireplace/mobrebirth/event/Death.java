...
public final class Death implements DeathHandler {
    ...
    private ResourceLocation getEntityBiomeId() {
        Holder<Biome> biome = livingEntity.getCommandSenderWorld().getBiomeManager().getBiome(livingEntity.blockPosition());

        return livingEntity.getCommandSenderWorld().registryAccess().registryOrThrow(Registry.BIOME_REGISTRY).getKey(biome.value());
    }
